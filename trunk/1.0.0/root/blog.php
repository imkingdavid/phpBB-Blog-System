...
if($config['blog_on'] == 0)
{
	if(!$auth->acl_get('a_'))
	{
		if($config['blog_off_msg'] != '')
		{
			trigger_error($config['blog_off_msg']);
		}
		else
		{
			trigger_error($user->lang['BLOG_DISABLED']);
		}
	}
	else
	{
		$template->assign_vars(array(
			'S_BLOG_DISABLED' => 1,
			'BLOG_DISABLED'	=> ($config['blog_off_msg'] != '') ? $config['blog_off_msg'] : $user->lang['BLOG_DISABLED'],
		));
	}
}
...
$template->assign_vars(array(
	'S_CAT_ENABLED'			=> ($cat_enabled) ? true : false,
	'S_TAGCLOUD_ENABLED'	=> (isset($tag_on)) ? true : false,
...
switch($action)
{
	...
	case 'index':		
		...
		{
			$blogrow = $blog->get_blog_data($blogrow['blog_id']);
			...
			$sql = 'SELECT COUNT(cmnt_id)
					...
					WHERE cmnt_blog_id = \'' . $blogrow['blog_id'] . '\'
					AND cmnt_approved = \'1\'';
...
//				'<font color="#' . $urow['user_colour'] . '">' . $urow['username'] . '</font>',
	...
	case 'view':
		...
		else
		{
			...
			$sql_limit = ($sql_limit > 100) ? 100 : $sql_limit;
			$pagination_url = append_sid($phpbb_root_path . 'blog.' . $phpEx . '?' . $config['blog_act_name'] . "=view&amp;id=$blog_id");
	...
	case 'post_blog':
		...
		$template->assign_block_vars('navlinks', array(
			...
			'U_VIEW_FORUM'	=> append_sid($phpbb_root_path . 'blog.' . $phpEx . '?' . $config['blog_act_name'] . '=post_blog'),
		));
		...
		else
		{
			...
			$template->assign_vars(array(
				...
				'U_ACTION'				=> append_sid($phpbb_root_path . 'blog.' . $phpEx . '?' . $config['blog_act_name'] . '=post_blog'),
			));
		}
		page_header($user->lang['BLOG']);
	...
	case 'post_comment':
		...
		if(request_var('submit', ''))
		{
			...
			trigger_error($sub . '<BR /><a href="' . $u_action . '">' . $message . '</a>');
		}
		page_header($user->lang['BLOG']);
		page_footer();
	break;
